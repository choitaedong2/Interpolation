...
class NAMOIimputation:
    def __init__(self, path = None, window_size = 50, use_gpu = torch.cuda.is_available(), dataframe = None):
        self.path = path
    ...
    def imputation(self, epoch = 250):
        ...
        self.train_data = torch.Tensor(self.data_batch)
        ...
        with trange(pretrain_epochs, file=sys.stdout) as tr:
            for e in tr:
                ...
                self.optimizer = torch.optim.Adam(
                    ...
                    lr=lr)
                train_loss = self.run_epoch(True, self.model, self.train_data, clip, self.optimizer, batch_size = batch_size, teacher_forcing=teacher_forcing)
                # print("asf")
